...
#include <fstream>
using namespace std;
G4Allocator<CsI_SDHit> CsI_SDHitAllocator;
...
const std::map<G4String,G4AttDef>* CsI_SDHit::GetAttDefs() const
{
    G4bool isNew;
    std::map<G4String,G4AttDef>* store 
    = G4AttDefStore::GetInstance("CsI_SDHit",isNew);
...
std::vector<G4AttValue>* CsI_SDHit::CreateAttValues() const
{
...
void CsI_SDHit::Print()
{
    G4cout << "  CsI#[" << fCellID << "] " << fDepositedEnergy/MeV << " (MeV)" << G4endl;
    
    
   ofstream dataFile ("CsI_out.txt", ios_base::app);
